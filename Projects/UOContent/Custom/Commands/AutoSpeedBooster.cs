...
namespace Joeku.ASB
{
	public class ASB_Main
	{
		...
		public static bool Initialized = true; // Has the script been initialized yet? (activated upon first player login)
		...
		public static void EventSink_OnLogin(LoginEventArgs e)
		{
			...
			if( e.Mobile.AccessLevel >= HasAccess ) // Does the player have access to the ASB?
			{
				if( GetASB(e.Mobile) ) // The player's ASB is enabled (default)...
				{
					...
					e.Mobile.SendMessage(0x120, "Automatic speed boost is enabled for this account. To disable it, use the \"AutoSpeedBooster\" command.");
				}
				else // The player's ASB is disabled
					e.Mobile.SendMessage(0x120, "Automatic speed boost is disabled for this account. To enable it, use the \"AutoSpeedBooster\" command.");
			}
			e.Mobile.SendMessage(String.Empty); // Skip a line
		}
